...
export default {
  ...
  methods: {
    Modify() {
      db.collection("gallery").doc(this.$store.state.galleryId).update({
        "name": this.name,
        "title": this.title,
  ...
  mounted() {
    if(this.$store.state.galleryId == 0){
      this.$router.replace("/cs/gallery")
    }
    db.collection("gallery").doc(this.$store.state.galleryId).get().then((data) => {
