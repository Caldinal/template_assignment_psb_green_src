<template lang="">
  <div class="basis-full">
    <div class="max-w-7xl mx-auto my-10">
      ...
      <template v-for="(e, index) in dataList" :key="index">
        <ul v-if="calculateNumber(totalLength, perPage, page, index) > 0" class="flex justify-between border-b text-center py-2 even:bg-gray-50 text-xs sm:text-sm">
          ...
          <li class="basis-4/12 text-center"><router-link :to="{ name: 'OnlineRead', query:{ docId: dataId[index] } }" @click="$store.commit('OnlineRead', dataId[index])">{{ e.title }}</router-link></li>
    ...
    <div v-if="$store.state.loginToken != null" class="flex justify-end pb-24">
      <router-link to="/cs/online/write" class="bg-indigo-400 hover:bg-indigo-600 focus:ring-indigo-400 py-2 px-4  text-white font-semibold rounded-md shadow-md focus:outline-none focus:ring-2 focus:ring-opacity-75 text-xs sm:text-sm">글쓰기</router-link>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    fetchTotalLength(){
      db.collection("online").get().then((data) => {
    ...
    fetchPost(){
      ...
      let query = db.collection("online").orderBy("date", "desc").limit(this.perPage)
