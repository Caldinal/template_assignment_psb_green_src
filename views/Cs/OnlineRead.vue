<template lang="">
  ...
  <!-- <div v-if="BoardContent.file" class="basis-9/12 mx-auto">
    <img :src="BoardContent.file" alt="첨부파일">
  </div> -->
  ...
  <div class="flex basis-9/12 mx-auto justify-end pb-24 gap-x-5 mt-5">
    <div class="mt-[5px]">
      <router-link to="/cs/online/list" class="bg-blue-400 hover:bg-blue-600 focus:ring-blue-400 py-2 px-4  text-white font-semibold rounded-md shadow-md focus:outline-none focus:ring-2 focus:ring-opacity-75 text-xs sm:text-sm">목록</router-link>
    </div>
    <div class="flex gap-x-5">
      <router-link to="/cs/online/modify" class="bg-green-400 hover:bg-green-600 focus:ring-green-400 py-2 px-4  text-white font-semibold rounded-md shadow-md focus:outline-none focus:ring-2 focus:ring-opacity-75 text-xs sm:text-sm">수정</router-link>
...
export default {
  ...
  methods: {
    Delete(){
      ...
      if(msg){
        console.log("!!")
        db.collection("online").doc(this.$route.query.docId).delete().then(() => {
          alert("삭제가 완료 되었습니다.")
          this.$router.replace("/cs/online")
        })
      }
    },
    GoodChk() {
      ...
      db.collection("online").doc(this.$route.query.docId).update({
        good: this.BoardContent.good + 1,
        goodChk : true
      }).then(() => {
        db.collection("online").doc(this.$route.query.docId).get().then((data) => {
          this.BoardContent = data.data()
        })
      })

      // this.BoardContent.good = this.BoardContent.good + 1;
    },
    noGoodChk() {
      ...
      // this.BoardContent.bad = this.BoardContent.bad + 1;
    },
  },
  mounted() {
    if(this.$store.state.onlineId == null){
      this.$router.replace("/cs/online")
    }
    db.collection("online").doc(this.$route.query.docId).get().then((data) => {
      this.BoardContent = data.data()
      ...
      db.collection("online").doc(this.$route.query.docId).update({
        hit: data.data().hit + 1, // 조회수 업데이트는 데이터를 가지고 온 곳 안쪽에 넣어야한다.
        // isUpdate: true
      })
    }).then(() => {
      db.collection("online").doc(this.$route.query.docId).get().then((update) => {
      ...
      return this.dateTime = formattedDate; // 날짜를 나타내는 함수
      })
    })
