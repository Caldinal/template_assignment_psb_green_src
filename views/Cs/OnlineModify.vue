...
export default {
  ...
  methods: {
    Modify() {
      db.collection("online").doc(this.$store.state.onlineId).update({
        "name": this.name,
        "title": this.title,
  ...
  mounted() {
    if(this.$store.state.onlineId == 0){
      this.$router.replace("/cs/online")
    }
    db.collection("online").doc(this.$store.state.onlineId).get().then((data) => {
